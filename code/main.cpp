...
#define APP_NAME TEXT("USBShield")
	...
	case WM_CREATE:
		{
			...
			HWND btnDeviceMgr = CreateWindow("BUTTON", "Device Manager",
			WS_VISIBLE | WS_CHILD,
			500, 62, 140, 30,
			hwnd, (HMENU)ID_DEVICEMGR, NULL, NULL);
			SendMessage(btnDeviceMgr, WM_SETFONT, (WPARAM)hFont, TRUE);
			...
			case ID_START:
				{
					...
					if (ServiceRunning())
					{
						MessageBox(hwnd, "Service already running", APP_NAME, MB_OK);
					}
					else
					{
						EnableWindow(btnStart, FALSE);
			...
			case ID_STOP:
				{
					...
					if (!ServiceRunning())
					{
						MessageBox(hwnd, "Service already stopped", APP_NAME, MB_OK);
					}
					else
					{
						EnableWindow(btnStart, FALSE);
			...
			case ID_UNINSTALL:
				{
					...
					hwnd,
					"Do you want to uninstall the service?",
					APP_NAME,
...
int nCmdShow)
{
	...
	RegisterClassEx(&wc);
	hMutex = CreateMutex(NULL, TRUE, "USBShieldGUI_Mutex");
	if (GetLastError() == ERROR_ALREADY_EXISTS)
	{
		MessageBox(NULL, "Application is already running.", APP_NAME, MB_OK | MB_ICONWARNING);
		return 0;
	}
